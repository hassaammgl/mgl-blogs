...
// 	const handleFileChange = (e) => {
// 		const file = e[0]; // Get the first file from the input
// 		if (!file) return;

// 		const reader = new FileReader();
// 		reader.onloadend = () => {
// 			const base64String = reader.result; // This is a data URL (e.g., "data:image/png;base64,...")
// 			console.log(base64String);

// 			// If you want to extract the raw Base64 data (without the data URL prefix):
// 			const base64Data = base64String.split(',')[1];

// 			// If you want to convert the Base64 data to a Uint8Array:
...
// 			// Set the image state (e.g., Base64 string or Uint8Array)
// 			setImage(buffer); // Or setImage(base64String) if you want to store the Base64 string
// 			setImageID(image._id); // Ensure `image._id` is defined
// 		};
...
// 						{image && (
// 							<img
// 								// src={`data:image/png;base64,${arrayBufferToBase64(image.data)}`}
// 								src={image.url}
...
// const Step5 = () => {
// 	const { imgByAi, contentByAi, title, description, category, image, content,  } =
// 		useBlogFormStore();
...
// 				"Content-Type": "application/json",
// 			},
// 			body: JSON.stringify({ imgByAi, contentByAi, title, description, category,image,  content, imageID: image._id, userEmail: user.emailAddresses[0].emailAddress }),
// 		});
// 		const data = await res.json();
// 		console.log(data);
// 	};

// 	const imgsrc = `data:image/png;base64,${arrayBufferToBase64(image.data)}`
...
// Step2: Image upload / AI generation
const Step2 = () => {
	const {
		setStep,
		step,
	...
	// For file uploads, convert file to buffer and create a blob URL for preview
	const handleFileChange = (files) => {
		...
		reader.onloadend = () => {
			...
			// Store the image as an object with the buffer, contentType and blobUrl
			setImage({
				data: buffer,
	...
	// For AI-generated images, convert the response data buffer from array to Uint8Array
	const handleGenerateImg = async () => {
		...
		const data = await res.json();
		// Convert the received data array to a Uint8Array if it exists
		if (data.data && data.data.data) {
			data.data = new Uint8Array(data.data.data);
		}
		...
		>
			...
			{imgByAi === false && (
				<>
					...
					{image && image.blobUrl && (
						<div className="mb-4 w-full justify-center items-center h-auto gap-4 border-2 border-dashed border-gray-500 rounded-lg p-4 flex">
							<img src={image.blobUrl} alt="Blog Img" />
						</div>
					)}
				</>
			)}
			...
			<div className="w-full flex justify-end">
				<NavigationButtons
					onNext={() => setStep(step + 1)}
					onPrev={() => setStep(step - 1)}
					// Disable Next if there is no image stored
					isNextDisabled={!image || !image.data}
					isPrevDisabled={step === 1}
				/>
			</div>
		</m.div>
...
// Step4: Content creation / AI generation
const Step4 = () => {
	const { setStep, step, setContentByAi, contentByAi, content, setContent } = useBlogFormStore();
	const [prompt, setPrompt] = useState("");
	const [res, setRes] = useState("");
		...
		>
			...
			{contentByAi === true && (
				<div className="mb-4">
					...
					{res !== "" && <Editor onChange={setRes} blogContent={res} />}
				</div>
			)}
			{contentByAi === false && (
				<div className="mb-4">
...
// Step5: Final preview and submission
const Step5 = () => {
	const { imgByAi, contentByAi, title, description, category, image, content } = useBlogFormStore();
	const { data: user } = useUserStore();

	// For preview, if an uploaded image has a blob URL use that;
	// otherwise use the AI-generated image data converted to base64.
	const imgsrc = image && image.blobUrl
		? image.blobUrl
		: image && image.data
			? `data:${image.contentType};base64,${arrayBufferToBase64(image.data)}`
			: "";
...
// Steps container: renders the appropriate step
const Steps = () => {
	const { step } = useBlogFormStore();
	const steps = {
		1: <Step1 />,
...
// AnimatedWrapper component for transitions
const AnimatedWrapper = ({ children }) => {
	return (
		<AnimatePresence mode="popLayout" initial exit={{ opacity: 0 }}>
			{children}
		</AnimatePresence>
	);
};
